...
// Register attaches policy handlers to mux.
...
// checkAdminAuth returns true if the request's Bearer matches AdminKey.
// When AdminKey is empty, all calls pass — useful in dev / tests.
func (s *Service) checkAdminAuth(r *http.Request) bool {
	if s.AdminKey == "" {
		return true
	}
	auth := r.Header.Get("Authorization")
	expected := "Bearer " + s.AdminKey
