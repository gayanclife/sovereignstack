...
type KeyStore struct {
	...
	// masterKey enables Phase C5 field encryption when non-nil. Set
	// via SetMasterKey after load; saveLocked transparently encrypts
	// Department/Team/Role on the way to disk.
	masterKey []byte
}
...
func (ks *KeyStore) saveLocked() error {
	// Phase C5: when a master key is configured, marshal an encrypted
	// snapshot rather than the in-memory plaintext. The in-memory map
	// itself stays plaintext so concurrent readers don't pay
	// decrypt-on-every-call. We deep-copy each profile and encrypt the
	// copy.
	users := ks.Users
	if len(ks.masterKey) > 0 {
		clone := make(map[string]*UserProfile, len(ks.Users))
		for id, p := range ks.Users {
			cp := *p
			if err := encryptFieldsLocked(ks.masterKey, &cp); err != nil {
				return fmt.Errorf("encrypt fields for %q: %w", id, err)
			}
			clone[id] = &cp
		}
		users = clone
