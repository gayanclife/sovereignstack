...
var keysAddCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		ipAllowCSV, _ := cmd.Flags().GetString("ip-allowlist")

		var ipAllowlist []string
		if strings.TrimSpace(ipAllowCSV) != "" {
			for _, entry := range strings.Split(ipAllowCSV, ",") {
		...
		if len(ipAllowlist) > 0 && role != keys.RoleService {
			return fmt.Errorf("--ip-allowlist requires --role service (got %q)", role)
		}
...
func init() {
	...
	keysAddCmd.Flags().StringP("role", "r", "user", "User role: user | admin | viewer | service")
	keysAddCmd.Flags().Float64P("rate-limit", "l", 100, "Rate limit (requests per minute)")
	keysAddCmd.Flags().String("ip-allowlist", "", "Comma-separated IPs/CIDRs for service-account auth (requires --role service)")
